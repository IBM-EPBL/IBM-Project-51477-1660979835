import necessary packages import cv2
import os
import numpy as np
from .utils import download_file

initialize = True net = None
...
class_file_abs_path = dest_dir + os.path.sep + class_file_name url = 'https://github.com/Nico31415/Drowning-
...
#if the person is drowning, the box will be drawn red instead of blue if label == 'person' and Drowning:
color = COLORS[0] label = 'DROWNING'
else:
color = COLORS[1]
...
if not os.path.exists(weights_file_abs_path): download_file(url=url, file_name=weights_file_name,
dest_dir=dest_dir)
...
indices = cv2.dnn.NMSBoxes(boxes, confidences, confidence, nms_thresh) bbox = []
label = [] conf = []
